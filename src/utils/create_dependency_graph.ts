...
export default function createDependencyGraph(rootGroup: MigrationGroup): {
  ...
  function extractBasePath(group: MigrationGroup) {
    ...
    const migrationsIndex = pathParts.findIndex(
  ...
  function constructAbsolutePath(
    ...
    for (let i = 1; i < pathParts.length; i++) {
      ...
      if (
        part?.includes("/") &&
        (part.includes(".sql") || part.includes(".js") || part.includes(".ts"))
      ) {
        filePart = part;
        break;
      }
  ...
  function resolveDependencyId(
    dep: string,
    currentGroupPath: string,
    isGroupDependency: boolean = false,
  ): string {
    if (dep.startsWith("::")) {
  ...
  function processGroup(group: MigrationGroup, parentPath: string = "") {
    ...
    const isActualFile =
      group.name.includes("/") && !group.name.startsWith("root");
        ...
        if (canonicalId === migrationId) {
          ...
          for (const dep of migrationInfo.migration.migrationDependencies) {
            ...
            migrationDependencies
              .get(migrationId)
              ?.migrationDependencies.add(depId);
          }
