...
export function organizeMigrations(
  ...
  const allMigrations = new Map<string, { migration: Migration; group: string }>();
    ...
    if (dependencies) {
      ...
      for (const groupDep of dependencies.groupDependencies) {
        for (const [otherMigrationId] of allMigrations) {
          if (otherMigrationId.startsWith(groupDep + '::') && otherMigrationId !== migrationId) {
            const cycle = checkCircularDependencies(otherMigrationId, currentPath);
            if (cycle) {
              return cycle;
            }
          }
  ...
  function topologicalSort(): void {
      ...
      for (const groupMigration of groupMigrations) {
        ...
        for (const dep of deps.migrationDependencies) {
          if (allMigrations.has(dep)) {
            ...
            inDegree.set(groupMigration, (inDegree.get(groupMigration) || 0) + 1);
          }
        }
        ...
        for (const groupDep of deps.groupDependencies) {
          for (const [otherMigrationId] of allMigrations) {
            if (otherMigrationId.startsWith(groupDep + '::') && otherMigrationId !== groupMigration) {
    ...
    if (organizedMigrations.length !== allMigrations.size) {
      const unprocessed = Array.from(allMigrations.keys()).filter(id => 
        !organizedMigrations.some(om => om.id === id)
      );
