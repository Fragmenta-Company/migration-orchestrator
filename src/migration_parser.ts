...
export default class MigrationParser {
  private static readonly _migrationStartRegex =
    /^--\s*\+migration:\s*(\w[\w-]*)/;
  private static readonly _migrationEndRegex = /^--\s*\+endmigration/;
  private static readonly _migrationRegexDescription = /^--\s*\+(.+)/;
  private static readonly _migrationRegexMigrationDependency =
    /^--\s*\+dependency:\s*([\w.\-:/]+)/;
  private static readonly _migrationRegexGroupDependency =
    /^--\s*\+groupdependency:\s*([\w.\-:/]+)/;
  private static readonly _migrationRegexTags = /^--\s*\+tags:\s*([\w\-,\s]+)/;
  private static readonly _migrationGroupStartRegex =
    /^--\s*\+group:\s*(\w[\w-]*)/;
  ...
  public async parseSingleFile(
    ...
    const parsedResult = await this.parseFile({
      filePath,
      content,
      serial: 1,
      singleFile: true,
    ...
    rootGroup.addGroup(parsedResult);
    ...
    const parsedResults = await Promise.all(
      fileOrder.map(async (filePath) => {
        ...
        const parsed = await this.parseFile({
          filePath,
          content,
          serial: fileOrder.indexOf(filePath) + 1,
        });
        return parsed;
      }),
    );
  ...
  private async parseFile({
    ...
    const rootGroup = new MigrationGroup(filePath, serial ?? 1, 1);
    ...
    const insideMigration = false;

    for (const [i, line] of lines.entries()) {
      match(line)
        .when(
          (l) => !!MigrationParser._migrationStartRegex.test(l),
          (l) => {
            const match = MigrationParser._migrationStartRegex.exec(l);
            if (match?.[1]) {
              if (currentMigration || insideMigration) {
                throw new Error(
                  `Found migration start without end in file: ${filePath} at line ${i + 1}`,
                );
              }
        ...
        .when(
          (l) => !!MigrationParser._migrationEndRegex.test(l),
          () => {
            if (currentGroup && currentMigration) {
              ...
              throw new Error(
                `Found migration end without start in file: ${filePath} at line ${i + 1}`,
              );
            }
          },
        )
        .when(
          (l) => !!MigrationParser._migrationGroupStartRegex.test(l),
          (l) => {
            const match = MigrationParser._migrationGroupStartRegex.exec(l);
            if (match?.[1]) {
              if (currentGroup) {
                const newGroup = new MigrationGroup(
                  match[1],
                  lastGroups.length + 1,
                  i + 1,
                );
                ...
                currentGroup = new MigrationGroup(
                  match[1],
                  lastGroups.length + 1,
                  i + 1,
                );
        ...
        .when(
          (l) => !!MigrationParser._migrationGroupEndRegex.test(l),
          () => {
            if (currentGroup) {
              if (lastGroupIndexes.length > 0) {
                const lastIndex = lastGroupIndexes.pop() ?? 0;
                if (!lastGroups[lastIndex]) {
                  throw new Error(
                    `Invalid last group index: ${lastIndex} in file: ${filePath} at line ${i + 1}`,
                  );
                }
                const lastGroup = lastGroups.splice(lastIndex, 1)[0] ?? null;
              ...
              throw new Error(
                `Found migration group end without start in file: ${filePath} at line ${i + 1}`,
              );
            }
          },
        )
        .when(
          (l) => !!MigrationParser._migrationRegexTags.test(l),
          (l) => {
            ...
            if (match?.[1]) {
              ...
              if (currentMigration) {
                ...
                throw new Error(
                  `Found migration tags without migration or group in file: ${filePath} at line ${i + 1}`,
                );
              }
            }
          },
        ...
        .when(
          (l) => !!MigrationParser._migrationRegexMigrationDependency.test(l),
          (l) => {
            const match =
              MigrationParser._migrationRegexMigrationDependency.exec(l);
            if (match?.[1]) {
              if (currentMigration) {
                ...
                throw new Error(
                  `Found migration dependency without migration in file: ${filePath} at line ${i + 1}`,
                );
              }
            }
          },
        ...
        .when(
          (l) => !!MigrationParser._migrationRegexGroupDependency.test(l),
          (l) => {
            const match =
              MigrationParser._migrationRegexGroupDependency.exec(l);
            if (match?.[1]) {
              if (currentMigration) {
                ...
                throw new Error(
                  `Group dependencies can't be added in the top level while parsing a single file.\n` +
                    `Found group dependency without migration or group in file: ${filePath} at line ${i + 1}`,
                );
              }
            }
          },
        ...
        .when(
          (l) => !!MigrationParser._migrationRegexDescription.test(l),
          () => {
            ...
            if (match?.[1]) {
              ...
              if (currentMigration) {
                ...
                throw new Error(
                  `Found description without migration or group in file: ${filePath} at line ${i + 1}`,
                );
              }
            }
          },
        ...
        .otherwise(() => {
          if (currentMigration) {
            ...
            throw new Error(
              `Found SQL outside of migration in file: ${filePath} at line ${i + 1}`,
            );
          }
        });
      ...
      if ((currentGroup || currentMigration) && i === lines.length - 1) {
        throw new Error(
          `File ended unexpectedly with open migration or group in file: ${filePath} at line ${i + 1}`,
        );
      }
      ...
      if (i === lines.length - 1 && !currentGroup && !currentMigration) {
        if (lastGroups.length > 0) {
          throw new Error(
            `File ended with open groups in file: ${filePath} at line ${i + 1}`,
          );
        }
        ...
        while (migrations.length > 0) {
          if (!rootGroup) {
            throw new Error(
              `Root group is null when trying to add migrations in file: ${filePath}`,
            );
          }

          rootGroup.addMigration(migrations.pop() as Migration);
        }
      }
    }
